...
class AddMovieToFavorites(private val transformer: CompletableTransformer, val moviesRepo: MoviesRepository) :
    ...
    override fun createSourceCompletable(params: Map<String, Any>?): Completable {
        params?.let {
            return moviesRepo.addMovieToFavorites(it[MOVIE_KEY] as MovieObj)
        } ?: kotlin.run {
